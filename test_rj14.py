...
selected_risks = st.sidebar.multiselect(
    "Selecione os Riscos:", ["Selecionar todos"] + [f"Risco {r}" for r in range(7)], default=["Selecionar todos"]
)
selected_concessions = st.sidebar.multiselect(
    "Selecione a Concessão:", ["Selecionar todos"] + malha_viaria['empresa'].unique().tolist(), default=["Selecionar todos"]
)
...
coordenadas_inicio = st.sidebar.text_input("Ponto Inicial (formato: lat, lon):", placeholder="-22.817762, -43.372672")
coordenadas_fim = st.sidebar.text_input("Ponto Final (formato: lat, lon):", placeholder="-22.664081, -43.222538")
...
with tabs[0]:
    ...
    # Capturar desenho e aplicar filtros
    map_output = st_folium(m, width=800, height=600, key="mapa_base")
    hexagonos_filtrados = hexagonos_h3.copy()

    # Aplicar filtro de desenho
    if "last_active_drawing" in map_output and map_output["last_active_drawing"]:
        try:
            geom = shape(map_output["last_active_drawing"]["geometry"])
    ...
    if "Selecionar todos" not in selected_risks:
        ...
        hexagonos_filtrados = hexagonos_filtrados[hexagonos_filtrados[coluna_risco_rounded].isin(selected_risk_values)]
